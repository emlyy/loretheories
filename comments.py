...
def get_comments():
    ...
    sql = text("SELECT u.username, c.message, c.posted_at, c.id, c.user_id, p.title FROM users u JOIN comments c ON c.user_id=u.id JOIN posts p ON c.post_id=p.id  WHERE c.post_id=:post_id")
